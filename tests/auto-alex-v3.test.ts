    ...
    it('auto-alex-v3 : ensure that it works', () => {
        ...
        for (let cycle = 0; cycle < redeem_cycle + 10; cycle++) {
            ...
            if (cycle > 1) {
                ...
                // console.log(simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-reward-cycle', [Cl.uint(simnet.blockHeight)], wallet_1).result);
                // console.log(simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-user-id', [], wallet_1).result);
                console.log('redeem-shares', simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-redeem-shares-per-cycle-or-default', [Cl.uint(cycle)], wallet_1).result.value);
                console.log('prev-shares-to-token', simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-shares-to-tokens-per-cycle-or-default', [Cl.uint(Math.max(cycle - 1, 0))], wallet_1).result.value);
                console.log('base-shares-to-token', simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-shares-to-tokens-per-cycle-or-default', [Cl.uint(Math.max(cycle - 33, 0))], wallet_1).result.value);
                // console.log('reward', simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-staking-reward', [Cl.uint(cycle)], wallet_1).result.value);
                console.log('staked', simnet.callReadOnlyFn('auto-alex-v3-endpoint', 'get-staker-at-cycle', [Cl.uint(cycle)], wallet_1).result.data);
