...
describe('auto-alex-v3', () => {
    it('auto-alex-v3 : ensure that it works', () => {
        ...
        for (let cycle = 0; cycle < redeem_cycle + 20; cycle++) {
            ...
            const reward_cycle = Number(simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-reward-cycle', [Cl.uint(simnet.blockHeight)], wallet_1).result.value.value);
            console.log('cycle', reward_cycle);
            console.log('pre-balance alex', simnet.callReadOnlyFn('SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex', 'get-balance', [Cl.principal(simnet.deployer + '.auto-alex-v3-endpoint-v2')], wallet_1).result);
            ...
            console.log('post-balance alex', simnet.callReadOnlyFn('SP102V8P0F7JX67ARQ77WEA3D3CFB5XW39REDT0AM.token-alex', 'get-balance', [Cl.principal(simnet.deployer + '.auto-alex-v3-endpoint-v2')], wallet_1).result);
            ...
            console.log('redeem-shares', simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-redeem-shares-per-cycle-or-default', [Cl.uint(reward_cycle - 1)], wallet_1).result.value);
            console.log('prev-shares-to-token', simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-shares-to-tokens-per-cycle-or-default', [Cl.uint(Math.max(reward_cycle - 2, 0))], wallet_1).result.value);
            console.log('base-shares-to-token', simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-shares-to-tokens-per-cycle-or-default', [Cl.uint(Math.max(reward_cycle - 34, 0))], wallet_1).result.value);                        
            ...
            console.log('staked', simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-staker-at-cycle', [Cl.uint(cycle + 1)], wallet_1).result.data);
            console.log('staked + 1', simnet.callReadOnlyFn('auto-alex-v3-endpoint-v2', 'get-staker-at-cycle', [Cl.uint(cycle + 2)], wallet_1).result.data);
            ...
            if (cycle == 2) {
                ...
                response = simnet.mineBlock([                      
                    // test redeems
                    tx.callPublicFn('auto-alex-v3-endpoint-v2', 'request-redeem', [Cl.uint(bal1)], wallet_1),
                    tx.callPublicFn('auto-alex-v3-endpoint-v2', 'request-redeem', [Cl.uint(bal2)], wallet_2),
                    tx.callPublicFn('auto-alex-v3-endpoint-v2', 'request-redeem', [Cl.uint(bal4)], wallet_4),
            ...
            if (cycle == 3) {
                ...
                response = simnet.mineBlock([
                    tx.callPublicFn('auto-alex-v3-endpoint-v2', 'request-redeem', [Cl.uint(bal3)], wallet_3),
                ]);
                expect(response[0].result).toHaveClarityType(ClarityType.ResponseOk);
            }
